...
for benchmark in args.input.iterdir():
    ...
    benchmark_meminfo_df = pl.scan_csv(benchmark.joinpath("repeat*/meminfo.out"), has_header=False, new_columns=["Measure", "Value"], dtypes=[pl.Utf8, pl.Float64])
    benchmark_df = pl.concat([benchmark_time_df, benchmark_perf_df, benchmark_meminfo_df])
